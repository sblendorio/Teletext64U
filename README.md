...
- NOS Teletekst (Dutch Teletext). 
- ARD-TEXT (German Teletext)
...
It uses the same internet feed as the mobile apps and websites. To make it feel even more like on TV, the header row is modified to show the current date with a page number top left. Entering page numbers works like as on a remote. Just type 3 digits and the requested page will be fetched, if available. Browse through sub pages with the cursor up/down keys and the Home key brings you to page 101 with the latest news. It also supports fast text, those four colored words you see at the bottom row. These are mapped to the C64U’s functions keys.
